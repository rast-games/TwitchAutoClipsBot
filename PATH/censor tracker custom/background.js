...
;// CONCATENATED MODULE: ./src/shared/js/background/browser-api.js
/**
...
/* harmony default export */ const browser_api = (getBrowser());
;// CONCATENATED MODULE: ./src/shared/js/background/constants.js
const TaskType = {
  PING: 'ping',
 ...
 */
function domain_getDomain(suffix, hostname, options) {
...
function factory_parseImpl(url, step, suffixLookup, partialOptions, result) {
    ...
    // Extract domain
    result.domain = domain_getDomain(result.publicSuffix, result.hostname, options);
...
//# sourceMappingURL=index.js.map
;// CONCATENATED MODULE: ./src/shared/js/background/utilities.js
...
const extractDomainFromUrl = url => {
  ...
  return es6_getDomain(url);
};
const extractHostnameFromUrl = url => {
  return getHostname(url);
};
const i18nGetMessage = (key, props = {}) => {
...
;// CONCATENATED MODULE: ./src/shared/js/background/ignore.js
...
class Ignore {
  ...
  async getAll() {
    const {
      ignoredHosts
    } = await browser_api.storage.local.get({
      ignoredHosts: []
    });
    return ignoredHosts;
  }
...
/* harmony default export */ const ignore = (new Ignore());
;// CONCATENATED MODULE: ./src/shared/js/background/pac.js
/**
...
;// CONCATENATED MODULE: ./src/shared/js/background/registry.js
...
class Registry {
  ...
  async getDomains() {
    const {
      ...
      useRegistry: true,
      ignoredHosts: [],
      customProxiedDomains: ["tiktok.com"]
    });
    if (!useRegistry) {
    ...
    const allDomains = [...domains, ...customProxiedDomains].filter(element => {
  ...
  async contains(url) {
    ...
    return !!(domains.includes(domain) || customProxiedDomains.includes(domain));
  }
...
/* harmony default export */ const registry = (new Registry());
;// CONCATENATED MODULE: ./src/shared/js/background/proxy.js
...
class ProxyManager {
  async getProxyingRules() {
    const {
      ...
      localProxyURI
    } = await browser_api.storage.local.get(['proxyServerURI', 'customProxyProtocol', 'customProxyServerURI', 'localProxyURI']);
  ...
  async requestIncognitoAccess() {
    if (browser_api.isFirefox) {
      const isAllowedIncognitoAccess = await browser_api.extension.isAllowedIncognitoAccess();
      if (!isAllowedIncognitoAccess) {
        await browser_api.browserAction.setBadgeText({
          text: '✕'
        });
        await browser_api.storage.local.set({
  ...
  async setProxy() {
    ...
    try {
      await browser_api.proxy.settings.set(config);
  ...
  async removeProxy() {
    await browser_api.proxy.settings.clear({});
  ...
  async ping() {
    ...
    if (!usingCustomProxy) {
      const {
        proxyPingURI
      } = await browser_api.storage.local.get('proxyPingURI');
  ...
  async controlledByOtherExtensions() {
    const {
      levelOfControl
    } = await browser_api.proxy.settings.get({});
  ...
  async takeControl() {
    const self = await browser_api.management.getSelf();
    const extensions = await browser_api.management.getAll();
    for (const {
      id,
      name,
      permissions
      ...
      if (permissions.includes('proxy') && name !== self.name) {
        ...
        await browser_api.management.setEnabled(id, false);
      }
    }
  }
  async removeCustomProxy() {
    ...
    await browser_api.storage.local.remove(['customProxyProtocol', 'customProxyServerURI']);
  }
  async removeLocalProxy() {
    await browser_api.storage.local.set({
      useLocalProxy: false
    });
    await browser_api.storage.local.remove(['localProxyURI']);
  }
...
/* harmony default export */ const proxy = (new ProxyManager());
;// CONCATENATED MODULE: ./src/shared/js/background/server.js
...
const fetchProxy = async ({
  ...
  try {
    ...
    if (fallbackProxyInUse) {
      ...
      await browser_api.storage.local.remove(['fallbackReason', 'fallbackProxyInUse', 'fallbackProxyError']);
    }
    await browser_api.storage.local.set({
      proxyPingURI,
      proxyServerURI,
    ...
    console.error(`Error on fetching proxy server: ${error}`);
  }
  console.groupEnd();
};
/**
...
const fetchIgnore = async ({
  ...
  fetch(ignoreUrl).then(response => response.json()).then(domains => {
    browser_api.storag.local.get({
      ignoredHosts: []
    }).then(({
      ignoredHosts
    }) => {
...
;// CONCATENATED MODULE: ./src/shared/js/background/settings.js
...
class Settings {
  ...
  getDangerIcon() {
    return browser_api.runtime.getURL('images/icons/128x128/danger.png');
  }
  changePageIcon(tabId, filename) {
    const title = this.getName();
    const path = browser_api.runtime.getURL(`images/icons/128x128/${filename}.png`);
    if (browser_api.isFirefox) {
      browser_api.browserAction.setIcon({
        tabId,
        path
      });
      browser_api.browserAction.setTitle({
        title,
        tabId
      });
    } else {
      browser_api.action.setIcon({
        tabId,
        path
      });
  ...
  async exportSettings() {
    const settings = await browser_api.storage.local.get(null);
    settings.domains = [];
  ...
  async importSettings(settings) {
    ...
    await browser_api.storage.local.set(settings);
  }
}
/* harmony default export */ const settings = (new Settings());
;// CONCATENATED MODULE: ./src/shared/js/background/task.js
...
class Task {
  ...
  async delay(name, {
    ...
    browser_api.alarms.create(name, {
  ...
  async schedule(tasks = []) {
    ...
    for (const {
      ...
      const alarm = await browser_api.alarms.get(name);
      if (alarm) {
...
/* harmony default export */ const task = (new Task());
;// CONCATENATED MODULE: ./src/shared/js/background/handlers.js
...
const showDisseminatorWarning = async url => {
  const hostname = extractDomainFromUrl(url);
  const {
    notifiedHosts,
    showNotifications
  ...
  if (showNotifications && !notifiedHosts.includes(hostname)) {
    await browser_api.notifications.create(hostname, {
      type: 'basic',
      title: settings.getName(),
      iconUrl: settings.getDangerIcon(),
      message: browser_api.i18n.getMessage('cooperationAcceptedMessage', hostname)
    });
    try {
...
const handleOnAlarm = async ({
  ...
  if (name === TaskType.PING) {
    ...
    await proxy.removeBadProxies();
  } else if (name === TaskType.SET_PROXY) {
    const proxyingEnabled = await proxy.isEnabled();
    if (proxyingEnabled) {
      await synchronize();
...
const handleBeforeRequest = async _details => {
  await proxy.ping();
...
const handleStartup = async () => {
  ...
  await task.schedule([{
    ...
    name: TaskType.SET_PROXY,
    minutes: 15
  }, {
    name: TaskType.REMOVE_BAD_PROXIES,
    minutes: 20
  }]);
  console.groupEnd();
};
const handleIgnoredHostsChange = async ({
  ignoredHosts = {}
} = {}, _areaName) => {
  if ('newValue' in ignoredHosts) {
    proxy.isEnabled().then(enabled => {
      if (enabled) {
        proxy.setProxy().then(proxySet => {});
      }
    });
  }
};
const handleCustomProxiedDomainsChange = async ({
  customProxiedDomains: {
    newValue
  } = {}
} = {}, _areaName) => {
  settings.extensionEnabled().then(enableExtension => {
    if (enableExtension && newValue) {
      proxy.isEnabled().then(async proxyingEnabled => {
        if (proxyingEnabled) {
          await proxy.setProxy();
        }
      });
...
const handleStorageChanged = async ({
  ...
  if (enableExtension || useProxy) {
    if (enableExtension) {
      ...
      browser_api.tabs.query({}).then(tabs => {
        for (const {
          id
        } of tabs) {
...
const handleInstalled = async ({
  ...
  const UPDATED = reason === browser_api.runtime.OnInstalledReason.UPDATE;
  const INSTALLED = reason === browser_api.runtime.OnInstalledReason.INSTALL;
...
const handleTabState = async (tabId, {
  status = 'loading'
} = {}, {
  url
} = {}) => {
  if (url && status === browser_api.tabs.TabStatus.LOADING) {
    settings.extensionEnabled().then(enabled => {
      if (enabled) {
        ignore.contains(url).then(async isIgnored => {
          registry.retrieveDisseminator(url).then(async ({
            url: disseminatorUrl,
            cooperationRefused
          }) => {
            if (disseminatorUrl) {
...
const handleTabCreate = async tab => {
  settings.extensionEnabled().then(enabled => {
    if (enabled) {
...
const handleProxyError = async ({
  ...
  const usingCustomProxy = await proxy.usingCustomProxy();
  ...
  if (proxyErrors.includes(error)) {
    ...
    if (currentProxyServer) {
      const badProxies = await proxy.getBadProxies();
...
;// CONCATENATED MODULE: ./src/shared/js/background/background.js



// Handle alarms for async tasks
browser_api.alarms.onAlarm.addListener(handleOnAlarm);
// Handle extension lifecycle events
browser_api.runtime.onStartup.addListener(handleStartup);
browser_api.runtime.onInstalled.addListener(handleInstalled);
browser_api.runtime.onUpdateAvailable.addListener(handleOnUpdateAvailable);
// Handle tab changes (e.g. new tab, tab closed)
browser_api.tabs.onUpdated.addListener(handleTabState);
browser_api.tabs.onCreated.addListener(handleTabCreate);
// Handle storage changes (e.g. settings)
browser_api.storage.onChanged.addListener(handleStorageChanged);
browser_api.storage.onChanged.addListener(handleIgnoredHostsChange);
browser_api.storage.onChanged.addListener(handleCustomProxiedDomainsChange);
if (browser_api.isFirefox) {
  // Firefox-specific handlers
  browser_api.proxy.onError.addListener(handleProxyError);
  browser_api.webRequest.onBeforeRequest.addListener(handleBeforeRequest, {
    urls: ['http://*/*', 'https://*/*'],
    types: ['main_frame']
  });
  browser_api.webRequest.onErrorOccurred.addListener(handleProxyError, {
    urls: ['<all_urls>']
  });
} else {
  // Chrome-specific handlers
  browser_api.webNavigation.onBeforeNavigate.addListener(handleBeforeRequest, {
    urls: ['http://*/*', 'https://*/*'],
    types: ['main_frame']
  });
  browser_api.proxy.onProxyError.addListener(handleProxyError);
}
/******/ })()
;