...
// NAMESPACE OBJECT: ./src/shared/js/background/server.js
...
;// CONCATENATED MODULE: ./src/shared/js/pages/options.js
...
(async () => {
  // For debugging purposes.
  window.server = server_namespaceObject;
  ...
  const optionsRegistryIsEmptyWarning = document.getElementById('optionsRegistryIsEmptyWarning');
  const optionsRegistryUpdateDatabaseButton = document.getElementById('optionsRegistryUpdateDatabaseButton');
  const optionsRegistryProxyingListButton = document.getElementById('optionsRegistryProxyingListButton');
  ...
  if (proxyStatus) {
    ...
    proxyStatus.innerText = browser_api.i18n.getMessage(proxyStatusMessage);
    proxyStatus.hidden = false;
  }
  if (browser_api.isFirefox) {
    const allowedIncognitoAccess = await browser_api.extension.isAllowedIncognitoAccess();
    const {
    ...
    if (howToGrantIncognitoAccess) {
      howToGrantIncognitoAccess.addEventListener('click', async () => {
        await browser_api.tabs.create({
          url: browser_api.i18n.getMessage('howToGrantIncognitoAccessLink')
        });
      });
    }
  ...
  } = browser_api.runtime.getManifest();
  if (version) {
    version.textContent = await browser_api.i18n.getMessage('optionsVersion', currentVersion);
  }
})();
/******/ })()
;