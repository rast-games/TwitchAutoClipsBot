...
const i18nGetMessage = (key, props = {}) => {
  return browser_api.i18n.getMessage(key);
};
...
;// CONCATENATED MODULE: ./src/shared/js/pages/popup.js
...
(async () => {
  ...
  openOptionsPage.addEventListener('click', async target => {
    await browser_api.runtime.openOptionsPage();
  });
  ...
  browser_api.storage.local.get(['currentRegionName', 'proxyServerURI', 'customProxyServerURI', 'proxyLastFetchTs', 'localProxyURI', 'activeProxyConfigName']).then(async ({
    currentRegionName,
    proxyServerURI,
    customProxyServerURI,
    proxyLastFetchTs,
  ...
  browser_api.tabs.query({
    ...
    const {
      useLocalProxy
    } = await browser_api.storage.local.get(['useLocalProxy']);
    proxy.alive().then(alive => {
      if (useLocalProxy) {
        return;
      }
      if (proxyingEnabled) {
    ...
    if (extensionEnabled) {
      ...
      if (browser_api.isFirefox) {
        browser_api.extension.isAllowedIncognitoAccess().then(allowedIncognitoAccess => {
          browser_api.storage.local.get({
            privateBrowsingPermissionsRequired: false
          }).then(({
      ...
      registry.retrieveDisseminator(currentHostname).then(({
        url: disseminatorUrl,
        cooperationRefused
      }) => {
        if (disseminatorUrl) {
          ...
          if (restrictionsFound) {
            if (cooperationRefused === false) {
              statusImage.setAttribute('src', 'images/icons/512x512/ori_blocked.png');
            }
          }
        }
      });
  ...
  browser_api.storage.local.get('backendIsIntermittent').then(({
    backendIsIntermittent = false
  }) => {
    backendIsIntermittentPopupMessage.hidden = !backendIsIntermittent;
  });
  proxy.controlledByOtherExtensions().then(controlledByOtherExtensions => {
    if (!browser_api.isFirefox && controlledByOtherExtensions) {
